...
import { ArrowLeft, Clock, Calendar, ArrowRight, Sparkles, BookOpen, TrendingUp } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-amber-500 selection:text-slate-950 font-sans">
      ...
      <section className="relative min-h-[90vh] md:min-h-screen flex flex-col justify-end pb-16 pt-28 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          ...
          >
            ...
            <p className="text-lg sm:text-xl md:text-2xl text-slate-300 max-w-3xl leading-relaxed mb-8 font-light">
            ...
            {featuredBlog && (
              <div className="p-4 sm:p-5 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 hover:border-amber-400/40 transition-all max-w-3xl group">
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 pb-6 border-b border-white/10">
          <div>
            <div className="text-xs uppercase tracking-widest text-amber-400 font-bold mb-2 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredBlogs.map((blog, index) => (
            <motion.article
              ...
              className="bg-slate-900/60 rounded-xl border border-white/10 overflow-hidden hover:border-amber-500/40 hover:shadow-2xl hover:shadow-amber-500/5 transition-all duration-300 group flex flex-col backdrop-blur-sm"
            >
              <Link to={`/blog/${blog.slug}`} className="block relative overflow-hidden aspect-[16/10]">
                <div className="absolute top-3.5 left-3.5 z-10 bg-slate-950/80 backdrop-blur-md px-3 py-1 text-[11px] font-bold tracking-tight text-amber-300 rounded-full border border-white/10">
                  {blog.category}
                </div>
                <img 
                  src={blog.imageUrl} 
                  alt={blog.title} 
                  className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700 brightness-95 group-hover:brightness-105"
                  loading="lazy"
                />
              </Link>
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <Link to={`/blog/${blog.slug}`} className="block mb-3">
                  <h3 className="text-xl font-bold text-white tracking-tight group-hover:text-amber-300 transition-colors line-clamp-2">
                    {blog.title}
                  </h3>
                </Link>
                ...
                <div className="pt-4 border-t border-white/10 flex items-center justify-between mt-auto">
                  <span className="text-xs text-slate-400 font-medium">By {blog.author}</span>
                  <Link 
                    to={`/blog/${blog.slug}`}
                    className="inline-flex items-center gap-1 font-bold text-xs text-amber-400 group-hover:text-amber-300 transition-colors uppercase tracking-wider"
                  >
        ...
        <div className="mt-20 p-8 sm:p-12 rounded-2xl bg-gradient-to-r from-blue-950/80 via-slate-900 to-amber-950/40 border border-blue-500/30 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <Link 
                to="/" 
                className="w-full sm:w-auto px-8 py-3.5 rounded-lg bg-white/10 hover:bg-white/15 text-white font-semibold text-sm border border-white/20 transition-all flex items-center justify-center gap-2"
              >
                <ArrowLeft className="w-4 h-4" /> Explore Skewer POS
              </Link>
            </div>
          </div>
        </div>
